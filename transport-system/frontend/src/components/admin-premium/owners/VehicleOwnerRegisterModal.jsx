...
export default function VehicleOwnerRegisterModal({ isOpen, onClose, onSuccess, owner }) {
    ...
    if (isOpen) {
      if (isEdit && owner) {
        setForm({
          ...
          mobile: owner.mobile || '',
          city: owner.city || '',
          owner_type: owner.owner_type || 'TRANSPORT_COMPANY',
          company_name: owner.company_name || '',
          email: owner.email || '',
          state: owner.state || 'Bihar',
  ...
  const validate = () => {
    ...
    if (!raw) errs.mobile = 'Phone number is required.';
    else if (!/^[6-9]\d{9}$/.test(raw)) errs.mobile = 'Please enter a valid 10-digit phone number.';
  ...
  const handleChange = (e) => {
    ...
    setForm(prev => ({
      ...prev,
      [name]: name === 'mobile' || name === 'alternate_mobile' ? formatPhone(value) : value,
    }));
  ...
  const handleKeyDown = (e) => {
    ...
    e.preventDefault();
    const map = { owner_name: phoneRef, mobile: cityRef };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!validate()) {
      const firstErr = ['owner_name', 'mobile', 'city'].find(f => errors[f]);
      if (firstErr === 'owner_name') ownerNameRef.current?.focus();
      else if (firstErr === 'mobile') phoneRef.current?.focus();
    ...
    try {
      ...
      let res;
      if (isEdit) {
        res = await adminAPI.updateVehicleOwner(owner.owner_id, payload);
      } else {
        res = await adminAPI.createVehicleOwner(payload);
      }

      if (res.data?.success) {
        showToast(isEdit ? '✓ Transport Owner updated successfully.' : '✓ Transport Owner registered successfully.');
        setForm(INITIAL_FORM);
          ...
          <div className="sticky top-0 z-10 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 px-7 py-5 flex items-center justify-between rounded-t-3xl">
            <div>
              <h2 className="text-xl font-bold tracking-tight text-gray-900 dark:text-white">
                {isEdit ? 'Edit Transport Owner' : 'Register Transport Owner'}
              </h2>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                {isEdit ? 'Update owner details below.' : 'Only 3 required fields. Fill optional details later.'}
              </p>
            </div>
            <button
              onClick={handleClose}
            ...
            <div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  />
                  {errors.mobile && <p className="mt-1.5 text-sm text-red-500 flex items-center gap-1.5">{errors.mobile}</p>}
                </div>

                <div>
                  <label className={labelCls}>City <span className="text-red-400">*</span></label>
                  <CityAutocomplete value={form.city} onChange={handleChange} onEnter={() => {}} inputClass={inputCls('city')} />
                ...
                <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input type="text" name="bank_account" value={form.bank_account} onChange={handleChange} placeholder="A/c number" className={inputCls('bank_account')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">IFSC Code</label>
                      <input type="text" name="bank_ifsc" value={form.bank_ifsc} onChange={handleChange} placeholder="IFSC" className={inputCls('bank_ifsc')} />
                    </div>
                    <div>
              ...
              >
                {submitting ? (
                  ...
                  <>
                    ...
                    {isEdit ? 'Update Owner' : 'Register Owner'}
                  </>
                )}
              </button>
            </div>
