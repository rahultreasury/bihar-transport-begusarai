...
-- Repairs all missing objects between prisma/schema.prisma and live PostgreSQL.
--
-- SAFETY: This script is ADDITIVE ONLY.
--   * No DROP statements
--   * No ALTER of existing columns
--   * No data mutation
--   * All operations guarded with IF NOT EXISTS / DO blocks
-- ============================================================================

-- 1. Create missing ENUMs (guarded)
DO $$
BEGIN
...
-- 2. Add missing columns to bookings (guarded)
...
-- 3. Add missing columns to drivers (guarded)
...
-- 4. Create reservations table (guarded)
...
-- 5. Create invoices table (guarded)
...
-- 7. Add foreign keys (guarded with DO blocks)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'reservations_booking_id_fkey' 
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'reservations_driver_id_fkey' 
        AND table_name = 'reservations'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'reservations_vehicle_id_fkey' 
        AND table_name = 'reservations'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'invoices_booking_id_fkey' 
